...
public class DiscRotator : MonoBehaviour
{
    ...
    private void CheckDiscDragStart(Vector3 pRaycastTarget)
    {
        ...
        if (_cubeBoxCollider.Raycast(ray, out RaycastHit hit, Vector3.Distance(_camera.transform.position, transform.position) * 2))
        {
			...
			_collisionPlane.distance = - (RubikCube.Dimensions * transform.lossyScale.x)/2.0f - Vector3.Dot(hit.normal, transform.position);
    ...
    private void DoDiscDragInProgress(Vector3 pRaycastTarget)
    {
        ...
        if (_collisionPlane.Raycast(ray, out float hit))
        {
            ...
            discIndex = Mathf.Clamp(discIndex + RubikCube.Dimensions / 2.0f, 0, RubikCube.Dimensions - 1);
    ...
    private void CheckMouseWheelDiscRotation(Vector3 pRaycastTarget, bool pClockWise)
    {
        ...
        if (_cubeBoxCollider.Raycast(ray, out RaycastHit hit, Vector3.Distance(_camera.transform.position, transform.position) * 2))
		{
            ...
            int layer = !positiveNormal ? 0 : (RubikCube.Dimensions - 1);
